...
int main(int argc, char **argv) {
    ...
    for (auto [edge, len] : edge_freq) sorted_edges.push_back({get<0>(edge), get<1>(edge), len});
    ...
    for (int i = 0; i < to_write.size(); i++) {
        cout << get<0>(to_write[i]) << " " << get<1>(to_write[i]) << " " << get<2>(to_write[i]) << endl;
    }
    return 0;
}