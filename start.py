...
def solve_tsp(nodes, distances):
    ...
    options = {
        "WLSACCESSID": str(os.getenv("WLSACCESSID")),
        "WLSSECRET": str(os.getenv("WLSSECRET")),
...
if __name__ == "__main__":
    ...
    for i in range(len(points)):
        for j in range(i + 1, len(points)):
            gg.add_edge(points[i], points[j])
    
    nnodes = list(range(npoints))
    point_to_index = {p: i for i, p in enumerate(points)}
    top_two_thirds = do_one_iter(gg, npoints)
    ndict = {
        (i, j): INF
        for i, j in combinations(nodes, 2)
    }
    for edge, freq in top_two_thirds:
        i = point_to_index[edge[0]]
        j = point_to_index[edge[1]]
        if i == j:
            continue
        ndict[(min(i, j), max(i, j))] = math.sqrt(sum((points[i][k] - points[j][k]) ** 2 for k in range(2)))
        # ndict[(j, i)] = math.sqrt(sum((points[i][k] - points[j][k]) ** 2 for k in range(2)))
    ...
    # tour, cost = solve_tsp(nnodes, ndict)
    # print(f"Optimal tour: {tour}")
    # print(f"Optimal cost: {cost:g}")
    # print("")