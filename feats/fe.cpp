...
int main(int argc, char* argv[]) {
    ...
    std::vector<double> minLeftWeight(numNodes, std::numeric_limits<double>::max());
    ...
    for (int i = 0; i < numEdges; ++i) {
        ...
        if (weight < minLeftWeight[u]) {
            minLeftWeight[u] = weight;
        }
    }
    ...
    for (const auto& edge : edges) {
        double normalizedWeight = minLeftWeight[edge.u]/(1+edge.weight);
