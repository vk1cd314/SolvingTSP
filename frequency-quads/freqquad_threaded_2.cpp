...
void process_edges(vector<array<int, 3>>& edge_list, int start, int end, int vertices, int QUAD_NO, bool& ends) {
...
int main(int argc, char** argv) {
    ...
    int num_threads = thread::hardware_concurrency(); // Get the number of threads supported by the system

    for (int turn = 0; turn < t_count; ++turn) {
        bool ends = false;

        // Split the workload between threads
        vector<thread> threads;
        int chunk_size = edge_list.size() / num_threads;
        for (int i = 0; i < num_threads; ++i) {
            int start_idx = i * chunk_size;
            int end_idx = (i == num_threads - 1) ? edge_list.size() : start_idx + chunk_size;

            threads.emplace_back(process_edges, ref(edge_list), start_idx, end_idx, vertices, QUAD_NO, ref(ends));
        }
        ...
        for (int i = 0; i < (int)edge_list.size(); ++i) {
            if (edge_list[i][2] >= 3 * QUAD_NO) {
                ...
                lock_guard<mutex> lock(mtx); // Locking for safe modification of shared resource
